...
function getBreedImage(name: string): string {
  // Use Dog CEO API for real breed images (free, no key required)
  const breedSlugMap: Record<string, string> = {
  ...
  const slug = breedSlugMap[name];
  if (slug) {
    return `https://dog.ceo/api/breed/${slug}/images/random`;
  }
  // fallback placeholder
  return `https://placedog.net/400/300?id=${Math.abs(name.charCodeAt(0) + name.charCodeAt(1))}`;
}
